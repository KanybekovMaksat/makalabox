...
export function LikeButton({ like }: LikeButtonProps) {
  ...
  const [isLiked, setIsLiked] = useState(false);
  const [localCount, setLocalCount] = useState(like.likeCount);
  ...
  useEffect(() => {
    if (userId && like.likes.includes(userId)) setIsLiked(true);
  }, [userId, like.likes]);

  const { mutate, isPending } = articleQueries.useLikeArticle(like.id);

  const handleLike = async () => {
    if (!isAuth) {
      router.push('/login'); // ← ваш путь к странице входа
      return;
    }
  ...
  return (
    <div className="flex items-center">
      <Tooltip content={isAuth ? (isLiked ? 'Больше не нравится' : 'Нравится') : 'Нужна авторизация'}>
        <Button
          isIconOnly
          radius="full"
          variant="light"
        ...
        >
          <Heart
            className={`w-5 h-5 transition-colors ${
              isLiked ? 'fill-blue-600 text-blue-600' : 'text-default-500'
            }`}
          />
        </Button>
      </Tooltip>

      <span className="text-sm font-medium text-default-400">{localCount}</span>

      {explode && (
        <ConfettiExplosion {...confettiConfig} onComplete={() => setExplode(false)} />
      )}
    </div>
  );
}