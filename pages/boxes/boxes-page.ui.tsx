...
// Вспомогательная функция для получения списка уникальных категорий
...
export const BoxesPage = () => {
  ...
  // 1. Состояния для поиска и фильтрации
  ...
  if (isError || !boxesData) {
    return (
      <div className="my-20 text-center">Ошибка загрузки данных.</div>
    );
  }
    ...
    <Container maxWidth="md" className="mx-auto mb-[65px]">
      <Box className="w-full px-4 py-6 bg-[white] rounded-lg">
        ...
        <Box className="mb-6 p-4 border border-gray-200 rounded-lg bg-gray-50">
          ...
          <Typography variant="body2" className="mb-2 font-semibold text-gray-700">
          ...
          <div className="flex flex-wrap gap-2">
            {categories.map((category) => (
              <Chip
                ...
                className={`cursor-pointer ${selectedCategory === category ? 'bg-pc-500 text-white' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-100'}`}
              />
            ))}
          </div>
        </Box>
        ...
        <div className="flex flex-col gap-3 items-center">
          {filteredBoxes.length === 0 && (
            <Typography variant="body1" className="text-center mt-8 text-gray-500">
          ...
          {filteredBoxes.map((box) => (
            ...
            <Link to={`/boxes/${box.id}`} key={box.id} className="w-full max-w-[650px]">
              {/* Карточка */}
              <Card className="flex items-start shadow-sm p-2 md:p-3 gap-3 w-full border border-gray-100 hover:bg-gray-50 transition duration-150">
                ...
                <CardContent className="flex-1 flex flex-col gap-0 p-0 pt-1">
                  ...
                  {/* Заголовок */}
                  <Typography variant="subtitle1" className="font-bold text-gray-900 leading-tight mb-1">
                    {box.name}
                  </Typography>
